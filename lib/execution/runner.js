const { KnexTimeoutError, timeout } = require('../util/timeout');
const { KnexAbortError, abortOnSignal } = require('../util/abort-signal');
...
class Runner {
  ...
  async query(obj) {
    ...
    // TODO: order with timeout?
    if (obj.abortSignal) {
      queryPromise = abortOnSignal(queryPromise, obj.abortSignal);
    }
    ...
    return queryPromise
      ...
      .catch((error) => {
        ...
        } else if (error instanceof KnexAbortError) {
          const { sql, bindings } = obj;

          return this.client
            .cancelQuery(this.connection)
            .catch((cancelError) => {
              // If the cancellation failed, we need to mark the connection as disposed so that
              // it gets destroyed by the pool and is never used again. If we don't do this and
              // return the connection to the pool, it will be useless until the current operation
              ...
              // cancellation failed
              throw Object.assign(cancelError, {
                message: `After abort signal fired, cancelling of query failed.`,
                sql,
                bindings,
              });
            })
            .then(() => {
              // cancellation succeeded, rethrow abort error
              throw Object.assign(error, {
                message: `Abort signal fired when running query.`,
                sql,
                bindings,
              });
            });
